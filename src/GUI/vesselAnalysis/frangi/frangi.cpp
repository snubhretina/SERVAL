...
void frangi2d(const Mat &src, Mat &maxVals, Mat &whatScale, Mat &outAngles, frangi2d_opts_t opts){
	...
	for (float sigma = opts.sigma_start; sigma <= opts.sigma_end; sigma += opts.sigma_step){
		...
		if (opts.BlackWhite)
		{
			//mul1.setTo(0, mul1 < 0);
			//mul2.setTo(0, mul2 < 0);
			for (int y = 0; y < mul1.rows; y++)
			for (int x = 0; x < mul1.cols; x++)
			{
				if (mul1.at<float>(y, x) < 0)
					mul1.at<float>(y, x) = 0;
				if (mul2.at<float>(y, x) < 0)
					mul2.at<float>(y, x) = 0;
			}
			//mu1(mu1<0) = 0;
			//mu2(mu2<0) = 0;
		...
		for (int y = 0; y < lambda2.rows; y++)
		for (int x = 0; x < lambda2.cols; x++)
		{
			if (lambda2.at<float>(y, x) == 0)
				lambda2.at<float>(y, x) = nextafterf(0, 1);
		}
	...
	for (int i=1; i < ALLfiltered.size(); i++){
		...
		for (int y = 0; y < whatScale.rows; y++)
		for (int x = 0; x < whatScale.cols; x++)
		{
			if (ALLfiltered[i].at<float>(y,x) == maxVals.at<float>(y,x))
				whatScale.at<float>(y, x) = sigma;
		}
